...
| `TEST-015` | Provisioning detail and secret reveal specs | partial request specs cover provisioning show HTML timeline, persisted step state, step partial replacement endpoint, warnings/errors, authz, 404 shell, and JSON compatibility; Stimulus consumer implements ActionCable + polling fallback; planned specs remain for retry, concurrent-job banners, authorized secret fetch, and TTL expiry | `AC-015` |

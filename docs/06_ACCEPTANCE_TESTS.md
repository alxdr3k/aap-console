...
| `TEST-016` | Config management UI specs | partial request/spec coverage exists for auth config HTML view/update/regenerate-secret flow, LiteLLM config HTML view/update/validation flow, and config-version HTML index/show/diff/rollback-diagnostics flow in `spec/requests/auth_configs_spec.rb`, `spec/requests/litellm_configs_spec.rb`, and `spec/requests/config_versions_spec.rb`; auth expansion UI remains planned | `AC-016` |

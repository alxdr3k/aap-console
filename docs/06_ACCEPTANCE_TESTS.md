...
| `TEST-016` | Config management UI specs | partial request/spec coverage exists for auth config HTML view/update/regenerate-secret flow, disabled future controls, and JSON compatibility in `spec/requests/auth_configs_spec.rb`; LiteLLM config and config-version diff/show/rollback diagnostics remain planned | `AC-016` |

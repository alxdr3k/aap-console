...
| `TEST-015` | Provisioning detail and secret reveal specs | partial request specs cover provisioning show HTML timeline, persisted step state, step partial replacement endpoint, warnings/errors, manual retry UX, concurrent-job banners, authz, 404 shell, and JSON compatibility; Stimulus consumer implements ActionCable + polling fallback; planned specs remain for authorized secret fetch and TTL expiry | `AC-015` |

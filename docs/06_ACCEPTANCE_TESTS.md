...
| `TEST-015` | Provisioning detail and secret reveal specs | partial request specs cover provisioning show HTML timeline, persisted step state, warnings/errors, authz, 404 shell, and JSON compatibility; planned system/channel/request specs remain for reconnect/polling fallback, retry, concurrent-job banners, authorized secret fetch, and TTL expiry | `AC-015` |

...
**retention target**: 180일 보관 후 일일 배치 Job(`ProvisioningJobsCleanupJob`)이 `completed` / `rolled_back` / `completed_with_warnings` 레코드를 아카이빙(S3)하고 삭제. `failed` / `rollback_failed`는 관리자 확인 전까지 보존. Current repo에는 아직 `ProvisioningJobsCleanupJob`이 없으며, retention cleanup은 `OPS-3A.4` / `AC-013`에서 닫아야 한다.

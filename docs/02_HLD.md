...
│  │ Turbo Streams│   │   Execute Job │   │   Channel         │ │
│  │ Stimulus     │   │ AppRegistry   │   │                   │ │
│  │              │   │   Webhook Job │   │                   │ │
│  └──────────────┘   └───────────────┘   └───────────────────┘ │
│                            │                                   │
│                     ┌──────▼───────┐                           │
...
**retention target**: 180일 보관 후 일일 배치 Job(`ProvisioningJobsCleanupJob`)이 `completed` / `rolled_back` / `completed_with_warnings` job과 step 레코드를 삭제한다. `failed` / `rollback_failed`는 관리자 확인 전까지 보존한다. 삭제 시 연결된 모든 `ConfigVersion`은 보존하고 `provisioning_job_id`만 null 처리한다. Production recurring schedule은 `config/recurring.yml`에 정의되어 있으며, 현재 구현은 S3 archive 없이 삭제 정책으로 `OPS-3A.4` / `AC-013`을 닫는다.
...
`audit_logs` 레코드는 **Service 계층에서만 명시적으로 작성** 한다. Model `after_commit` / `after_save` 콜백으로 자동 작성하지 않는다.

| 근거 | 설명 |
|------|------|
| **행위자(user_sub) 전파** | `audit_logs.user_sub` 는 요청을 시작한 실제 사용자 ID 가 필요하다. Model 콜백은 `Current.user_sub` 에 의존하게 되는데, 배경 Job(`ProvisioningExecuteJob` 등)에서 레코드가 수정될 때 `Current` 가 비어있으면 감사 로그가 익명으로 기록되거나 누락된다. Service는 호출 시점의 `current_user` 를 명시적으로 주입받아 이 문제를 원천 차단한다. |
| **감사 대상 액션 명시** | 동일 Model 저장이라도 액션 의미가 다르다 (예: Project.update 가 이름 변경인지 설정 변경인지). Model 콜백은 이를 구분하지 못하므로 Service가 `action: "project.rename"` / `"project.config_update"` 를 명시적으로 지정한다. |
| **롤백 의도 보존** | 프로비저닝 롤백 중 발생하는 Model 변경(status 전이 등)은 감사 로그에 남기지 않아야 한다. Model 콜백이면 구분 불가. |

**구현**: Service 의 `call` 메서드 마지막에 `AuditLog`를 생성한다. 필드는 `organization`, `project`, `user_sub`(주입된 행위자), `action`, `resource_type`, `resource_id`, `details`. `details` 에는 평문 시크릿이 들어가지 않도록 `rules/security.md` 의 금지 목록을 적용한다.
...
| GET | `/provisioning_jobs/:id/secrets` | 완료된 Job의 일회성 시크릿 조회 endpoint. OIDC client secret은 10분 TTL cache에서 읽고, cache miss/미완료 Job은 `{}` 반환 | Project `write`+ |
...
> - 클라이언트 측: 표준 `fetch(..., { method: "POST", body: JSON.stringify(payload) })` + `response.body.getReader()` (`ReadableStream`) 로 청크를 읽는다. `EventSource` 는 GET-only이므로 사용하지 않는다.
> - `max_duration: 120s` 를 컨트롤러에서 강제. Puma `worker_timeout`(기본 60s) 보다 길어야 한다면 Puma 설정을 `worker_timeout: 130` 이상으로 상향하여 worker가 SSE 도중 kill되지 않도록 맞춘다. 둘 중 하나라도 어긋나면 스트림이 중간에 끊긴다.
> - `max_tokens`는 서버에서 상한 4096로 클램핑 (UI 값이 이를 초과하면 경고). LiteLLM Config에서 더 낮은 모델별 상한을 설정하면 그쪽이 우선.
> - 클라이언트 disconnect 감지 시 LiteLLM 업스트림 요청도 즉시 취소 (`Faraday` 의 `on_complete` + `response.stream.closed?` 체크, 또는 `IOError: closed stream` rescue).
> - 동시 SSE 연결 수는 Project당 3개로 제한 (`Rails.cache` 카운터). 초과 시 429 반환. `ActionController::Live` 가 스레드를 점유하므로 Puma `max_threads` 와 함께 고려한다.
...
| 3 | `db_cleanup` | — (Console DB) | `project_permissions` + `project_api_keys` 삭제 + Project status → `deleted` |
...
> **100ms 폴링의 전제**: 이 튜닝은 Console이 **단일 Pod + Recreate 전략** 으로 배포된다는 전제(ADR-005) 하에서만 안전하다. 모든 publisher/subscriber 가 동일 SQLite 파일을 공유하므로 100ms 폴링이 로컬 I/O 로 해결된다. 향후 LiteFS replica 확장 등으로 **쓰기 노드와 읽기 노드가 분리** 되면 replica 측 polling 은 네트워크 지연 + WAL 전파 지연이 누적되므로 `polling_interval: 0.5s` 수준으로 상향하거나 Redis 기반 adapter 로 교체한다.
...
현재 구현 상태: `KeycloakClientCreate`의 OIDC client secret은
...
`ProjectApiKeys::RevealCache`를 통해 같은 TTL/metadata-guard 원칙을 재사용한다.
...
3. 브라우저는 프로비저닝 현황 페이지가 `completed` 상태를 감지하면 `GET /provisioning_jobs/:id/secrets` 를 호출한다. 인증 설정 Secret 재발급/PAK 발급 browser flow는 같은 Project의 `/auth_config` HTML redirect 뒤 렌더 단계에서 project-scoped cache payload를 읽는다. 모든 경로는 **TTL(10분) 내에 반복 조회 가능** 하며, 동일 Project에 대한 서로 다른 세션(여러 관리자, 재방문) 도 권한이 있다면 모두 접근할 수 있다.
4. 컨트롤러는 매 fetch/render마다 요청 사용자의 해당 Project `write`+ 권한을 다시 검증하고, 캐시가 존재하면 값을 응답하거나 렌더한다. 캐시는 TTL 만료 시에만 자동 삭제되며 **응답 즉시 무효화하지 않는다** — "누가 먼저 fetch 했는지" 로 나머지 관리자가 차단되는 구조를 피하기 위함.
...
- **HTTP 클라이언트**: Faraday (`faraday ~> 2.9`) + `faraday-retry ~> 2.2` (2.x에서 retry는 별도 gem) + `faraday-net_http ~> 3.1` (기본 adapter). JSON 요청/응답 (`request :json` / `response :json`), 에러 자동 raise (`response :raise_error`).
- **에러 처리**: `Faraday::Error` 계열 (`ClientError`, `ServerError`, `TimeoutError` 등) → 도메인 `ApiError` 서브클래스(status, body, upstream 포함) 로 변환.
...
- **Retry Middleware**: `faraday-retry` 의 `request :retry` 미들웨어를 탑재한다. 대상은 idempotent 요청(GET, HEAD) 과 `Retry-After` 헤더가 있는 429/503 응답만. 쓰기 요청의 재시도는 Step 레벨(§5.3 + `X-Idempotency-Key`) 에서 제어한다 (Faraday-level retry는 `methods: [:get, :head]` 로 제한).
...
| `get_config(org:, project:, service:)` | `GET /api/v1/config` | FR-6 (조회) |
| `get_secrets_metadata(org:, project:, service:)` | `GET /api/v1/secrets/metadata` | FR-6 (조회) |
| `get_history(org:, project:, service:)` | `GET /api/v1/history` | FR-8 (조회) |

> **읽기 엔드포인트 범위**: 위 3개 `GET` 엔드포인트는 PRD §3 "Console → Config Server 인터페이스" 표의 쓰기 4종과 달리 **Config Server 가 일반 서비스들에게도 제공하는 공개 읽기 API** 를 Console이 재사용하는 것이다. Console 전용 Admin API 가 아니라 PRD 수준에서 특별히 명시되지 않았으며, 현재 설정/시크릿 메타데이터/이력 페이지 렌더링 용도로만 사용된다. Config Server 사양 변경 시 이 3개 엔드포인트만 HLD를 업데이트하면 되고 PRD 수정은 불필요하다.
>
...
- `joined_at` 업데이트는 해당 멤버십당 **최초 1회**만 수행 (`WHERE joined_at IS NULL`). 매 로그인마다 수정하지 않는다.
...
**기존 사용자가 추가 Org에 가입하는 경우**: 이미 Console에 로그인한 이력이 있는 사용자가 새 Org의 멤버로 추가되면, 새 `org_memberships` 레코드는 `invited_at = now, joined_at = NULL` 로 생성된다. 사용자가 **다음 번 로그인** 하거나 세션이 살아있는 상태에서 해당 Org를 처음 접근하면 `joined_at` 이 그 시점으로 채워진다.

즉 `joined_at` 은 **"사용자가 Console 에 처음 로그인한 시각"이 아니라 "이 멤버십이 활성화된 시각"** 을 나타낸다 — 멤버십 단위로 해석한다. `SessionsController#create` 는 세션 생성 시 해당 `user_sub` 의 모든 멤버십 중 `joined_at IS NULL` 인 레코드를 일괄 갱신한다. Org 접근이 지연되는 경우를 줄이기 위해 세션 로그인 시점에 eager backfill하는 방식이다.
...
`app/models/current.rb` — `ActiveSupport::CurrentAttributes` 서브클래스로 요청 단위 전역 저장소를 제공한다. `user_sub`, `realm_roles`, `request_id` 속성을 보유하며 `super_admin?` predicate를 노출한다. 요청 종료 시 자동 리셋되며, 로그 태그(`sub`)와 감사 로그(`changed_by_sub`)에서도 동일 객체를 참조한다.
...
> **Thread 안전성 주의**: `Current`는 Thread-local 저장소다. §5.2에서 기술했듯 병렬 프로비저닝 Thread는 상위 요청 컨텍스트를 상속받지 못하므로, Thread 시작 전에 필요한 값(`user_sub`)을 로컬 변수로 캡처하여 `Thread.new { |sub| Current.set(user_sub: sub) { ... } }` 블록으로 전달해야 한다. `Current.set(attrs) { block }` 형태는 Rails 7.1+ 에서 제공되며 블록 종료 시 이전 값으로 자동 복원되므로, Thread 내부에서 예외가 발생해도 누수가 없다.

#### 사용자 정보 조회 (Keycloak API)

Console DB에 이메일/이름을 저장하지 않으므로, 멤버 목록 등 UI 표시 시 `KeycloakClient`의 사용자 조회 메서드(Section 7.2)를 사용한다.

**배치 조회 패턴**: 멤버 목록처럼 N개의 `user_sub`에 대한 정보를 한 번에 조회할 때는 `get_users_by_ids(user_subs:)`를 사용한다. Keycloak Admin API는 **여러 ID를 한 번에 조회하는 안정적인 엔드포인트가 없다** — `GET /users?q=id:{sub}` 는 특정 버전에서 조용히 ID 필터를 무시했고, `GET /users?search=id:UUID1 UUID2` (PR #37901) 는 `search` 파라미터 파싱 변경 이후 버전별로 동작이 불일치한다 ([Keycloak Issue #42479](https://github.com/keycloak/keycloak/issues/42479)). 현재 사용 중인 26.4.5 fork 포함 어떤 버전에도 의존하지 않도록, 구현은 **per-ID 엔드포인트(`GET /admin/realms/{realm}/users/{id}`) 를 병렬 호출** 하는 방식으로 고정한다.

구현 방침: `Concurrent::FixedThreadPool`(기본 5, Keycloak 부하 + Puma `max_threads` 고려해 5~10 범위로 제한)에서 `user_subs`를 병렬 GET하고 `{sub => user_attrs}` 해시로 합친다. 풀은 호출 완료 시 즉시 종료한다.

- 404 응답은 "삭제된 사용자" 로 간주하여 해당 키를 결과 해시에서 제외한다 (멤버십 레코드가 남아있어도 UI에는 "삭제된 사용자" 로 표시).
- 네트워크 오류는 즉시 전파 — 멤버 목록 화면 전체가 에러 상태로 전환된다. 부분 성공 허용 시 멤버 이름이 간헐적으로 빠지는 UX 가 발생하므로 도입하지 않는다.
- 대량 조회 (>50명 Org) 는 N번의 병렬 GET을 발생시키므로, 향후 부하 이슈가 관찰되면 요청 단위 캐시(아래 **캐싱**) 로 완화한다.
...
| Config Server → Console | App Registry 벌크 조회 | `GET /api/v1/apps?all=true` | `Authorization: Bearer <CONSOLE_INBOUND_API_KEY>` |

...
> **범위 표기 원칙**: FR 섹션은 최종 제품 요구사항을 설명하고, §9는 제품 우선순위만 정의한다. 구현/노출 순서와 현재 상태는 [`04_IMPLEMENTATION_PLAN.md`](./04_IMPLEMENTATION_PLAN.md)가 관리한다. 아직 해당 Phase가 아닌 기능(예: Playground, 다중 인증 방식, 전용 관리자 대시보드)은 UI 설계에는 포함할 수 있으나 실제 제품에서는 숨김 또는 disabled 상태로 노출한다.
...
| **App Registry 변경** | `POST /api/v1/admin/app-registry/webhook` | App 등록/수정/삭제 시 Config Server의 인메모리 인증 캐시 갱신. 일반 운영 시에는 **fire-and-forget + async retry** 방식으로 전송 — Console DB 저장 후 비동기 전송하며, Config Server 다운 시에도 Console CRUD는 정상 완료. **단, 프로비저닝 파이프라인 내에서는 동기 실행** (완료 대기 후 후속 단계 진행) |
...
| **PAK 검증** | Config Server (런타임) | `POST /api/v1/project_api_keys/verify` — Config Server가 inbound 요청 인증을 위해 Project API Key 평문을 Console에 전달하면, Console이 SHA-256 해시를 비교해 활성 PAK 여부와 소속 organization/project를 응답한다. `CONSOLE_INBOUND_API_KEY` 헤더로 호출 주체를 인증한다 |
...
| **Langfuse 트레이싱 연동** | Playground 요청에 `metadata.source: "playground"` 태그를 추가하여 Langfuse에서 실제 트래픽과 구분 가능. 각 대화의 Langfuse trace URL을 UI에서 직접 링크로 제공 |
...
| **Config Server 인증/인가** | **Config Server** | `Authorization: Bearer <API_KEY>` 헤더로 인증. API Key에 id/name/scope/permissions가 포함되어 호출자 식별 및 권한 검증을 수행. API Key는 Console 환경변수(`CONFIG_SERVER_API_KEY`)로 관리 |
...
| Config Server Admin API 응답 | ≤ 5초 | `POST /api/v1/admin/changes` 단일 호출 |
...
- 동일 Project에 대한 프로비저닝 작업은 직렬 실행을 보장하여 중복 처리를 방지한다
- 서로 다른 Project 간의 작업은 상호 차단하지 않는다
- 각 프로비저닝 단계는 멱등하게 설계한다 (FR-7.2)
...
Step 1b. App Registry 등록 [Config Server Admin API]
  └─ POST /api/v1/admin/app-registry/webhook — App ID 등록 (재시도 포함, 완료 대기)
...
- **Deployment** (Recreate 전략) + PVC + Litestream Sidecar
- 단일 인스턴스 배포 (SQLite 단일 writer 제약). 업그레이드 시 짧은 다운타임(수십 초) 허용
...
이 섹션은 제품 우선순위만 정의한다. 구현 상태, gate 상태, evidence, next work의
canonical ledger는 [`04_IMPLEMENTATION_PLAN.md`](./04_IMPLEMENTATION_PLAN.md)가
관리한다.

| Priority | Product outcome | Related FR |
|---|---|---|
| P0 | Organization / Project 셀프서비스 관리와 Console DB RBAC | FR-1, FR-2, FR-3 |
| P0 | OIDC, Langfuse, LiteLLM, Config Server 기본 프로비저닝 자동화 | FR-4, FR-5, FR-6, FR-7 |
| P0 | 운영 안정성 gate: release smoke, health check, rollback 진단/복구 | FR-7, FR-8, FR-9 |
| P1 | SAML/OAuth/PAK 범위 확정 및 구현 | FR-4 |
| P2 | Playground 및 관리자 고도화 기능 | FR-10 |

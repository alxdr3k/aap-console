...
**구현**: Service 의 `call` 메서드 마지막에 `AuditLog.create!(organization:, project:, user_sub: @current_user_sub, action:, resource_type:, resource_id:, details:)` 를 호출한다. `details` 에는 평문 시크릿이 들어가지 않도록 `rules/security.md` 의 금지 목록을 적용한다.
...
- **HTTP 클라이언트**: Faraday (`faraday ~> 2.x`). JSON 요청/응답 (`faraday` built-in `request :json` / `response :json`), 에러 자동 raise (`response :raise_error`).
- **Gemfile**:
  ```ruby
  gem "faraday", "~> 2.9"
  gem "faraday-retry", "~> 2.2"  # Faraday 2.x에서 Retry 미들웨어는 별도 gem
  gem "faraday-net_http", "~> 3.1"  # 기본 adapter. Rails 기본 Net::HTTP 사용
  ```
...
def get_users_by_ids(user_subs:)
  ...
  # 스레드 풀 크기는 Keycloak 부하와 Puma max_threads 를 고려해 5~10 사이로 제한
  pool = Concurrent::FixedThreadPool.new(KEYCLOAK_BATCH_CONCURRENCY)  # default 5
  futures = user_subs.uniq.map do |sub|
    Concurrent::Promises.future_on(pool) { [sub, get_user(user_sub: sub)] }
  end

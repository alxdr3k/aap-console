...
> **⚠ 비공식 API 리스크**: tRPC는 Langfuse 웹 UI의 내부 API로 공식 public contract가 아니다. Langfuse 버전 업그레이드 시 procedure 시그니처가 예고 없이 변경될 수 있다. **완화 전략**: (1) 통합 테스트에서 tRPC 호출 검증 자동화, (2) Langfuse 업그레이드 시 Console 호환성 사전 테스트 필수, (3) Langfuse EE 전환 시 Admin API (`ADMIN_API_KEY` + Org API Key)로 마이그레이션 — [ADR-002](./adr-002-langfuse-api-strategy.md) 참조.
...
> 상세 결정 근거는 [10.3 설계 결정](#103-왜-console-db-전량-인가인가) 참조.
...
### 10.3 왜 Console DB 전량 인가인가

> 상세 비교는 [ADR-004](./adr-004-auth-authz-separation.md) 참조.
...
### 10.4 왜 tRPC(세션 쿠키)인가

> 상세 비교는 [ADR-002](./adr-002-langfuse-api-strategy.md) 참조.

| 방식 | 런타임 | 안정성 | OSS 지원 | 결정 |
|------|:------:|:------:|:--------:|------|
| **tRPC (세션 쿠키)** | O | 낮음 | O | **채택** |
| Admin API (`ADMIN_API_KEY`) | O | 높음 | EE만 | 미채택 (EE 전환 시 마이그레이션 대상) |
| Headless Init (환경변수) | 기동 시만 | 높음 | O | 미채택 (런타임 관리 불가) |
| DB 직접 조작 | O | 매우 낮음 | O | 미채택 |

**결정 근거**: Langfuse OSS에는 프로그래밍 방식의 공식 관리 API가 없다. tRPC는 Langfuse UI가 내부적으로 사용하는 API이므로 기능적으로 완전하지만, 비공식 API라는 리스크를 수반한다. 통합 테스트 자동화와 Langfuse 업그레이드 시 사전 호환성 검증으로 완화한다.

### 10.5 왜 직접 API 호출인가

> 상세 비교는 [ADR-003](./adr-003-external-api-integration-strategy.md) 참조.

| 방식 | 장점 | 단점 | 결정 |
|------|------|------|------|
| **직접 API 호출 (Ruby Client)** | 즉시 실행. 세밀한 에러 핸들링. Saga 패턴 호환 | Drift detection 없음 (FR-9로 완화) | **채택** |
| Terraform Provider | 선언적. State 기반 drift detection | Langfuse Provider 부재. 실행 오버헤드. Saga 패턴과 상충 | 미채택 |

**결정 근거**: Langfuse Terraform Provider가 존재하지 않아 Custom Provider 개발이 불가피하고, Terraform 실행 사이클(init→plan→apply)의 레이턴시가 프로비저닝 UX에 부정적. Console의 목적은 인프라 lifecycle이 아닌 서비스 설정 자동화이므로 직접 API 호출이 적합.

### 10.6 왜 Console DB 스냅샷인가
...
| Console → Config Server | 설정/시크릿 CRUD | Admin API (`/api/v1/admin/*`) | `X-App-ID` 헤더 |
| Console → Config Server | App Registry 알림 | Webhook (`/api/v1/admin/app-registry/webhook`) | `X-App-ID` 헤더 |

...
| **Config Server 인증/인가** | **Config Server** | `Authorization: Bearer <API_KEY>` 헤더로 인증. `X-App-ID` 헤더는 요청 컨텍스트 식별용 (비밀 정보 아님). API Key는 Console 환경변수(`CONFIG_SERVER_API_KEY`)로 관리 |

...
>
> **Langfuse 트레이싱**: LiteLLM은 `metadata` 필드를 Langfuse에 전달한다 (`litellm_params.metadata`). Console이 `source: "playground"`를 주입하면, Langfuse에서 `metadata.source = "playground"` 필터로 테스트 트래픽만 조회할 수 있다. LiteLLM 응답의 `x-litellm-trace-id` 헤더(또는 `_litellm_trace_id` 필드)로 Langfuse trace URL을 구성하여 UI에 링크한다.
>
> **인증 방식 검증 범위**: OIDC/SAML/OAuth 인증 플로우는 사용자의 앱 서비스와 Keycloak 간의 동작이므로 Playground에서 직접 검증할 수 없다. Playground가 확인하는 것은 **LiteLLM Config(모델 라우팅, 가드레일)과 Langfuse 트레이싱이 정상 적용되었는지**이다. Keycloak Client가 정상 생성되었는지는 FR-9 Health Check에서 커버한다.

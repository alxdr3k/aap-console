...
| `UI-5C.4` | `P0-M5` | `UI` | `UI-5C` | Unified Project configuration edit (§8.5.1): single form combining auth + LiteLLM + metadata, single PATCH triggers single Update provisioning job; convert §8.8/§8.11 to read-only detail views with `[설정 편집]` CTA | `UI-5C.1` / `UI-5C.2` | `AC-024` / `TEST-024` | `defined` | `planned` | `docs/ui-spec.md#851-project-설정-통합-편집--fr-346`, `app/controllers/projects_controller.rb`, `app/views/projects/edit.html.erb`, `app/javascript/controllers/dirty_tracker_controller.js`, `spec/requests/projects_spec.rb` | Backend `Projects::UpdateService` already aggregates dirty_attributes; only `project_update_params` permit + new edit view + read-only conversion of existing edit pages required |

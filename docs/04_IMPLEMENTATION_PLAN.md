...
| `SEC` | Secret reveal and zero-store product path | `SEC-5B` | `landed` | Provisioning secret reveal cache path is landed; auth/PAK follow-up remains in `UI-5C.1` / `AUTH-6A.3` |
...
| `AUTH-6A.1` | `P1-M1` | `AUTH` | `AUTH-6A` | SAML metadata UI, validation, disabled-to-enabled transition, provisioning coverage | `AUTH-4A.1` / `UI-5C.1` | `AC-017` / `TEST-017` | `defined` | `planned` | `DEC-003`, `docs/ui-spec.md#811-인증-설정-편집--fr-4` | Backend/API already accepted, but metadata field contract and export surface still need scope tightening before implementation |
| `AUTH-6A.2` | `P1-M1` | `AUTH` | `AUTH-6A` | OAuth/PKCE UI, public-client constraints, redirect URI validation | `AUTH-4A.1` / `UI-5C.1` | `AC-017` / `TEST-017` | `defined` | `planned` | `DEC-003`, `docs/01_PRD.md#fr-4-인증-체계-자동-구성` | Backend/API already accepted, but final form contract is still thinner than PAK UI scope |
| `AUTH-6A.3` | `P1-M1` | `AUTH` | `AUTH-6A` | PAK list/issue/revoke UI with one-time reveal and audit feedback | `AUTH-4A.2` / `SEC-5B.1` | `AC-017` / `TEST-017` | `defined` | `ready` | `app/controllers/project_api_keys_controller.rb`, `docs/ui-spec.md#811-인증-설정-편집--fr-4`, `app/javascript/controllers/secret_reveal_controller.js` | Uses accepted PAK API and existing one-time reveal UX foundation; next implementation leaf on the auth-config surface |
...
- PAK one-time reveal UI remains planned with `AUTH-6A.3`; `SEC-5B.1` covers the current OIDC client secret reveal path.

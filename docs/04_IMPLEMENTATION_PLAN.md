...
| `AUTH` | 인증 방식 확장과 PAK | `AUTH-6A` / `AUTH-6B` | `planned` | P0-M4 backend/API gate accepted; PAK product UI landed; SAML/OAuth product UI (`AUTH-6A`) is ready; auth_type migration (`AUTH-6B`) is planned after AUTH-6A |
...
| `AUTH-6A.1` | `P1-M1` | `AUTH` | `AUTH-6A` | SAML metadata UI, SP metadata URL copy, provisioning coverage | `AUTH-4A.1` / `UI-5C.1` | `AC-017` / `TEST-017` | `defined` | `ready` | `DEC-003`, `DEC-005`, `docs/ui-spec.md#811-인증-설정-편집--fr-4` | Contract defined: SP metadata URL copy only (no XML download); auth_type change via AUTH-6B; remaining field decisions deferred to implementation |
| `AUTH-6A.2` | `P1-M1` | `AUTH` | `AUTH-6A` | OAuth/PKCE UI, public-client display, redirect URI validation | `AUTH-4A.1` / `UI-5C.1` | `AC-017` / `TEST-017` | `defined` | `ready` | `DEC-003`, `DEC-005`, `docs/01_PRD.md#fr-4-인증-체계-자동-구성` | Contract defined: public/confidential display only; reuse uri_list controller; PKCE server validation adds HTTPS requirement (localhost exempt) and fragment rejection |

...
| `PROV-5C.6` | `P0-M5` | `PROV` | `UI-5C` | Conditional update step seeding: derive seeded steps from the dirty external subset (skip `keycloak_client_update` for LiteLLM-only updates, scope `health_check` to the touched services) so the persisted job timeline matches HLD §5.6 / AC-024 expectations | `UI-5C.4` | `AC-024` / `TEST-024` | `defined` | `planned` | `app/services/provisioning/step_seeder.rb`, `app/services/provisioning/steps/health_check.rb`, `spec/services/provisioning/step_seeder_spec.rb` | Today the runtime steps short-circuit on irrelevant fields, but the seeded step list still includes all three update steps. Aligns persisted timeline + retry surface with the dirty subset. |

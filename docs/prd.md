...
- **Project 설정 반영** (3경로):
  ...
  3. **Helm Chart 반영 (aap-helm-charts)**: SealedSecret 등 배포 시 반영이 필요한 설정
...
| **Langfuse Chart** | (해당 없음 — Retention은 전역 설정으로 Console에서 관리하지 않음) | |
...
| **aap-helm-charts** | 해당 App 관련 SealedSecret 등을 Git에서 제거 후 커밋 |
...
| **트레이싱 연동** | LiteLLM이 Config Server의 메타데이터에서 ID를 확인 → K8s Secret에서 실제 SK/PK를 조회하여 Langfuse 트레이싱 자동 연동 |
...
| 항목 | 상세 |
|------|------|
| **자동 생성** | Keycloak Client Secret, PAK 등을 SealedSecret YAML로 자동 변환 |
| **레포 저장** | 생성된 SealedSecret YAML을 `aap-helm-charts` 레포에 자동 커밋 |
| **K8s 반영** | K8s 클러스터의 SealedSecret Controller가 복호화하여 각 서비스에 Secret 주입 |

> **참고**: Langfuse SK/PK는 SealedSecret이 아닌 Config Server가 `kubectl apply`로 K8s Secret을 직접 생성하여 관리한다 (FR-5 참조).
...
- Keycloak Client Secret, PAK는 생성 시 UI에 일회성 표시 후 Console 미저장. SealedSecret으로 K8s에 전달
...
Step 2. Terraform Workspace 초기화 (App ID 기반)
  ...
  └──▶ Step 4b. aap-helm-charts 반영 [Git 커밋]
        └─ Secret → 해당 Chart (SealedSecret: Keycloak Client Secret, PAK 등)
  │
  ▼

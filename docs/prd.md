...
| **Config Server** | 설정 파일 서빙 및 시크릿 관리 서버. Git으로 데이터와 이력을 관리하며, LiteLLM이 동적으로 Config를 가져가는 출처. Langfuse SK/PK는 메타데이터만 Git에 저장하고 실제 값은 `kubectl apply`로 K8s Secret에 저장. 별도 레포(`aap-config-server`)로 관리 |
...
>
> **인가**: Console → Config Server 요청 시 서비스 인증 및 사용자 권한(Org/Project 범위)을 검증한다 (비기능 요구사항 6.1 참조).
...
| 항목 | 상세 |
|------|------|
| **이력 관리** | Project별 설정 변경 시마다 버전 기록 (Config Server Git 커밋 기반) |
...
| 계층 | 검증 내용 |
|------|-----------|
| **서비스 인증** | Console이 Config Server의 정당한 호출자인지 검증. 사전 공유된 API Key 또는 mTLS 기반 서비스 간 인증 |
| **사용자 권한 전파** | Console이 요청 시 사용자 식별 정보 및 대상 Org/Project를 함께 전달. Config Server는 해당 사용자가 대상 리소스에 대한 `write` 이상 권한을 보유하는지 검증 |
| **요청 범위 제한** | Config Server는 요청된 Org/Project 범위 내의 설정만 변경 허용. 다른 Project의 설정에 접근하거나 변경하는 요청은 거부 |
...
Step 2. Terraform Workspace 초기화 (App ID 기반)
  ...
  │ ── 설정 반영 ──
  │
  ├──▶ Step 4a. Config Server 반영 [API 호출 + Git 커밋 + Webhook]
...
**병렬 처리**: Step 3a ~ 3b는 상호 독립적이므로 병렬 실행 가능.
...
| **Config Server** | Nginx + Git + kubectl (`aap-config-server`) | LiteLLM Config 서빙 및 Langfuse SK/PK K8s Secret 관리. DB 없이 Git으로 데이터/이력 관리. Helm Chart는 `aap-helm-charts`에 포함 |

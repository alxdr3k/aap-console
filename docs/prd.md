...
| **Config Server** | 설정 관리 애플리케이션 서버. API를 통해 설정 변경 요청을 수신하고, Git으로 데이터와 이력을 관리하며, Nginx로 config 파일을 static 서빙한다. LiteLLM이 동적으로 Config를 가져가는 출처. Langfuse SK/PK는 메타데이터만 Git에 저장하고 실제 값은 `kubectl apply`로 K8s Secret에 저장. 별도 레포(`aap-config-server`)로 관리 |
...
```
        ...
        └─────┬──────┘  └─────┬──────┘  └──────────────┘
              ...
              │         fetch ▼│  webhook
    ┌─────────▼─────┐  ┌──────┴──────────┐
    │ Terraform     │  │  Config Server  │  App Server + Nginx + Git (aap-config-server)
    │ Orchestrator  ├─▶│  · LiteLLM Config│  모델 라우팅, 가드레일, S3 경로
    │ (Bg Jobs)     │  │  · Langfuse Meta │  SK/PK 메타(ID,org,proj) — 값은 K8s Secret
    └─────────────────┘  └────────┬────────┘
                                │ kubectl apply
                                ▼ (SK/PK → K8s Secret)
┌─ K8s Cluster ──────────────────────────────────────────┐
...
- **Project 설정 반영** (2경로):
  ...
  2. **동적 Config 반영 (Config Server)**: LiteLLM Config(모델 라우팅, 가드레일, S3 경로) 및 Langfuse SK/PK 메타데이터를 Config Server Git에 커밋 + SK/PK 실제 값은 `kubectl apply`로 K8s Secret 생성 → Console이 LiteLLM에 reload webhook 전송 → LiteLLM이 Config Server에서 fetch하여 즉시 반영 (재배포 불필요)

**Config Server 동작 방식**:
- 애플리케이션 서버가 Console의 API 요청을 수신하여 Git 커밋, K8s Secret 관리 등을 수행
- Nginx가 Git 레포의 config 파일을 static file로 서빙 (LiteLLM이 fetch하는 엔드포인트)
...
Organization (조직)
 └── Project (App ID 발급)
      ...
      │    └── SDK Key 발급 (PK/SK) → Config Server
      │         ├── 메타데이터(ID, org, proj) → Git 저장
      │         └── 실제 SK/PK → K8s Secret (kubectl apply)
      │
...
| **Config Server** | 해당 App의 LiteLLM Config 및 Langfuse 메타데이터를 Git에서 제거 후 커밋 → reload webhook. 메타 ID에 해당하는 K8s Secret도 삭제 |
...
| **SDK Key 발급** | Public Key, Secret Key 자동 발급. Console은 SK/PK를 **저장하지 않고** 즉시 Config Server에 전달 |
| **Config Server 전달** | Console → Config Server API 호출. Config Server는 SK/PK 메타데이터 객체(ID, org, proj 등)를 생성하여 Git에 JSON/YAML로 저장 (실제 SK/PK 값 미포함) |
| **K8s Secret 생성** | Config Server가 메타데이터 ID를 이름으로 하는 K8s Secret을 `kubectl apply`로 생성하여 실제 SK/PK 값을 저장 |
| **트레이싱 연동** | LiteLLM이 Config Server API를 호출하여 SK/PK를 조회 (Config Server가 메타 ID로 K8s Secret에서 실제 값을 조회하여 응답) → Langfuse 트레이싱 자동 연동 |

> **설계 원칙**: Git에는 시크릿 값이 올라가지 않는다. 실제 SK/PK는 K8s Secret에만 존재하며, Config Server의 메타데이터 ID를 통해 참조한다.
>
> **인가**: Console이 사용자 RBAC 권한을 검증한 후 Config Server에 요청을 전달한다. Config Server는 서비스 인증 및 요청 범위만 검증한다 (비기능 요구사항 6.1 참조).
...
| **롤백 — Config Server** | Config Server Git 이력을 기반으로 LiteLLM Config 및 Langfuse 메타데이터를 이전 버전으로 복구 → reload webhook 전송 |
...
- Langfuse SK/PK는 Console 미저장. Config Server가 메타데이터(Git)와 실제 값(K8s Secret)을 분리 관리
...
**Console → Config Server 요청 인증/인가**:

| 계층 | 검증 주체 | 검증 내용 |
|------|-----------|-----------|
| **사용자 권한 검증** | **Console** | API 요청 수신 시 Console이 자체 DB(PostgreSQL)의 RBAC 데이터를 기반으로 요청 사용자가 대상 Org/Project에 대한 `write` 이상 권한을 보유하는지 검증. 권한 미달 시 Config Server에 요청을 전달하지 않음 |
| **서비스 인증** | **Config Server** | Console이 정당한 호출자인지 검증. 사전 공유된 API Key 또는 mTLS 기반 서비스 간 인증 |
| **요청 범위 제한** | **Config Server** | 요청에 포함된 Org/Project 범위 내의 설정만 변경 허용. 다른 Project의 설정에 접근하거나 변경하는 요청은 거부 |
...
| **Config Server Git** | Config Server가 Git 커밋 시 파일 단위 또는 App ID 단위 잠금을 적용하여 동시 커밋 충돌 방지. 충돌 발생 시 자동 재시도 |
...
Step 2. Terraform Workspace 초기화 (App ID 기반)
  ...
  └──▶ Step 3b. Langfuse 리소스 생성 [Langfuse API]
         ├─ Langfuse Org/Project 생성
         └─ SDK Key (PK/SK) 발급 (Console은 미저장, 즉시 Config Server로 전달)
  │
  │ ── Step 3a/3b 완료 대기 후 설정 반영 ──
  │
  ▼
Step 4. Config Server 반영 [API 호출 + Git 커밋 + Webhook]
  ├─ LiteLLM Config (모델 라우팅, 가드레일, S3 경로/Retention) → Git 커밋
  ├─ Langfuse SK/PK → Config Server API 호출
  │    ├─ 메타데이터 객체(ID, org, proj) → Git 커밋 (실제 값 미포함)
  │    └─ SK/PK 실제 값 → kubectl apply로 K8s Secret 생성 (이름: 메타 ID)
...
| **Config Server** | App Server + Nginx + Git + kubectl (`aap-config-server`) | Console API 요청 수신 → Git 커밋 + K8s Secret 관리. Nginx로 LiteLLM Config static 서빙. DB 없이 Git으로 데이터/이력 관리. Helm Chart는 `aap-helm-charts`에 포함 |
...
| Config Server | App Server + Nginx + Git (`aap-config-server`) | 설정 관리 API + LiteLLM Config 서빙 |

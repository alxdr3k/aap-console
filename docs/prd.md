...
타 부서 사용자가 **셀프서비스 방식**으로 프로젝트(App)를 등록하면, AAP의 공유 서비스들(Keycloak, LiteLLM, Langfuse 등)에 해당 App을 지원하기 위한 **설정이 자동으로 추가**되는 관리 콘솔을 구축한다.
...
| **App (App ID)** | 타 팀이 등록하는 프로젝트 단위. 공유 서비스들의 설정 격리 단위 |
...
| **조회** | App 목록 및 상세 정보(각 서비스에 추가된 설정 현황) 확인 |
| **수정** | App 설정 변경 (인증 방식, 모델 라우팅, Retention 정책 등) |
| **삭제** | App 삭제 시 각 서비스에서 해당 App 관련 설정 자동 제거 (Terraform destroy) |
...
- App 생성 요청 후 전체 설정 완료까지 목표: 5분 이내
- Console UI 페이지 로드 시간: 2초 이내
- 실시간 로그 스트리밍 지연: 1초 이내

### 6.3 확장성

- 동시 다수 App 생성 요청 처리 가능 (Background Job 큐 기반)
...
사용자가 Console에서 App을 생성하면, 기존 공유 서비스들에 해당 App을 위한 설정이 추가된다.
...
Step 2. Terraform Workspace 초기화 (App ID 기반)
  │
  ├──▶ Step 3a. Keycloak 설정 추가
  │     └─ 선택한 인증 방식에 따라 Client 생성 또는 PAK 발급
  │
  ├──▶ Step 3b. Langfuse 설정 추가
...
- App 삭제 요청 시 해당 App의 Terraform Workspace에 대해 `terraform destroy` 자동 실행 (각 서비스에서 App 관련 설정 제거)
...
| **스토리지** | S3 (Terraform Provider) | Langfuse용 App별 전용 경로 및 Lifecycle 관리 |
...
- 설정 변경 이력 관리 및 버전 롤백
- Health Check 자동화
- App 삭제 시 설정 제거 (destroy) 자동화

### Phase 4: 고도화

- 다중 인증 방식 전체 지원 (SAML, OAuth, PAK)
- 관리자 대시보드 (전체 App 현황, 서비스 설정 상태)

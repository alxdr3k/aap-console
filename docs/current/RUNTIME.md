...
`health_check`는 warning-only step이다. 실패해도 rollback하지 않고
`completed_with_warnings`를 만든다.
...
- `Orchestrator`는 완료 시 `ProvisioningChannel.broadcast_to`로 broadcast한다.
...
| Flow | State |
|---|---|
| Health check service-specific assertions | Placeholder 수준. `SPIKE-001` 참고 |

...
| Secret reveal | OIDC client secrets and PAK plaintext are stored only in `Rails.cache`: provisioning completion uses `Provisioning::SecretCache` keyed by provisioning job, auth-config regeneration uses `AuthConfigs::SecretRevealCache` keyed by project, and browser PAK issuance uses `ProjectApiKeys::RevealCache` keyed by project. All keep a 10-minute TTL and metadata guard |

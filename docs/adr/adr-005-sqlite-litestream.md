...
- **인프라 거버넌스 부담**: 서버형 DB 도입 시 아키텍처 검토·보안 심의 등 사전 절차 수반. 과제 일정 내 소화 어려움
...
3. 서버형 DB 미사용으로 아키텍처 상 외부 의존성이 드러나지 않아, 인프라 거버넌스 절차를 최소화할 수 있음
4. Rails 8 Solid 스택과 자연스러운 통합
5. Litestream으로 데이터 안전성 확보 (S3 실시간 백업, 자동 복원)
